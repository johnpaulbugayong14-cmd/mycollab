...
let ticketHistoryListenerGeneration = 0;
const ticketDocumentUnsubscribes = new Map();
...
function cleanupTicketHistoryListener() {
  if (!ticketHistoryUnsubscribe) {
    console.log('[TICKET_REALTIME] LISTENER CLEANUP (no active listener)');
    return;
  }

  console.log('[TICKET_REALTIME] LISTENER CLEANUP');
  try {
    ticketHistoryUnsubscribe();
  } catch (error) {
    console.warn('[TICKET_REALTIME] LISTENER CLEANUP ERROR:', error);
  }
...
function refreshTicketHistoryFromExternalEvent() {
  if (!userEmail) return;
  console.log('[TICKET_HISTORY_FETCH] external refresh triggered');
  void refreshTicketHistoryNow();
}

window.addEventListener('mycollab-ticket-refresh', refreshTicketHistoryFromExternalEvent);
  ...
  if (window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform()) {
    import('@capacitor/app').then(({ App }) => {
      ...
      App.addListener('resume', () => {
        syncPresence(true, { force: true });
        reconnectTicketHistoryOnResume();
      });
    }).catch((error) => {
...
window.submitTicket = async function () {
  ...
  try {
    ...
    if (submissionMessage) submissionMessage.textContent = 'Ticket submitted successfully. It now appears in Ticket History.';
  } catch (displayError) {
    console.warn('Ticket created, but the history view could not refresh immediately:', displayError);
  }
...
function renderAllTicketHistory(docs) {
  latestTicketSnapshotDocs = docs;
  renderTicketHistory('ticketHistory', 'ticketHistoryEmptyState', docs);
  renderTicketHistory('maintenanceTicketHistory', 'maintenanceTicketHistoryEmptyState', docs);
}

async function refreshTicketHistoryNow() {
  if (!userEmail) return;

  try {
    const snapshot = await getDocsFromServer(collection(db, 'tickets'));
    const docs = [];
    snapshot.forEach(ticketDoc => docs.push(ticketDoc));
    latestTicketSnapshotDocs = docs;
    renderAllTicketHistory(docs);
  } catch (error) {
    console.warn('Manual ticket refresh failed:', error);
  }
}

function startTicketHistoryAutoRefresh() {
  if (ticketHistoryRefreshTimer) return;
  ticketHistoryRefreshTimer = setInterval(() => {
    if (document.visibilityState === 'visible' && userEmail) {
      console.log('[TICKET_HISTORY_FETCH] periodic refresh triggered');
      void refreshTicketHistoryNow();
    }
  }, 15000);
}

function ensureTicketHistoryListener(containerId = "ticketHistory", emptyStateId = "ticketHistoryEmptyState") {
  if (!userEmail) {
    console.log('[TICKET_REALTIME] LISTENER CREATE SKIPPED: no userEmail');
    return;
  }

  if (ticketHistoryUnsubscribe) {
    console.log('[TICKET_REALTIME] LISTENER ACTIVE: skipping duplicate listener creation');
    return;
  }

  ticketHistoryListenerGeneration += 1;
  const listenerGeneration = ticketHistoryListenerGeneration;
  console.log('[TICKET_REALTIME] LISTENER CREATE', {
    userEmail,
    containerId,
    emptyStateId,
  ...
  ticketHistoryUnsubscribe = onSnapshot(collection(db, "tickets"), (snapshot) => {
    if (listenerGeneration !== ticketHistoryListenerGeneration) {
      console.log('[TICKET_REALTIME] stale listener callback ignored', { listenerGeneration, currentGeneration: ticketHistoryListenerGeneration });
      return;
    }

    console.log('[TICKET_REALTIME] SNAPSHOT RECEIVED', { size: snapshot.size, ids: snapshot.docs.map(docSnap => docSnap.id) });
    snapshot.docChanges().forEach((change) => {
      const id = change.doc.id;
      if (change.type === 'added') {
        console.log('[TICKET_REALTIME] ADDED', id);
      } else if (change.type === 'modified') {
        console.log('[TICKET_REALTIME] MODIFIED', id);
      } else if (change.type === 'removed') {
        console.log('[TICKET_REALTIME] REMOVED', id);
      }
    });
    ...
    console.error('[TICKET_REALTIME] LISTENER ERROR:', error);
    console.error('[TICKET_REALTIME] Error code:', error?.code);
    console.error('[TICKET_REALTIME] Error message:', error?.message);
    cleanupTicketHistoryListener();

    const container = document.getElementById(containerId);
    const emptyState = document.getElementById(emptyStateId);
    if (emptyState) emptyState.style.display = 'block';
    if (container) {
      container.innerHTML = '<p style="color:#f87171; text-align:center;">Unable to load ticket history right now. Reconnecting...</p>';
    }

    setTimeout(() => {
      if (userEmail && !ticketHistoryUnsubscribe) {
        console.log('[TICKET_REALTIME] LISTENER RECONNECT');
        loadTicketHistory(containerId, emptyStateId, true);
      }
    }, 1000);
  });
}

function watchCreatedTicket(ticketId) {
  ticketDocumentUnsubscribes.get(ticketId)?.();
  const unsubscribe = onSnapshot(doc(db, 'tickets', ticketId), (ticketSnapshot) => {
  ...
  ticketDocumentUnsubscribes.set(ticketId, unsubscribe);
}
...
function loadTicketHistory(containerId = "ticketHistory", emptyStateId = "ticketHistoryEmptyState", forceRefresh = false) {
  ...
  if (!container) {
    console.warn('Ticket history container not found:', containerId, emptyStateId);
    return;
  }
  ...
  if (forceRefresh) {
    console.log('[TICKET_HISTORY_FETCH] forcing server refresh for ticket history');
    getDocsFromServer(collection(db, "tickets")).then((snapshot) => {
      const docs = [];
      snapshot.forEach(ticketDoc => docs.push(ticketDoc));
      latestTicketSnapshotDocs = docs;
      renderAllTicketHistory(docs);
    }).catch((error) => {
      console.error('Error forcing a server refresh for tickets:', error);
    });
  }

  if (emptyState) emptyState.style.display = "none";
  ensureTicketHistoryListener(containerId, emptyStateId);
}
...
// Make ticket history refresh functions available globally
window.loadTicketHistory = loadTicketHistory;
window.refreshTicketHistory = refreshTicketHistoryNow;

function reconnectTicketHistoryOnResume() {
  if (document.visibilityState !== 'visible' || !userEmail) return;

  console.log('[TICKET_REALTIME] APP RESUME');
  cleanupTicketHistoryListener();
  loadTicketHistory('ticketHistory', 'ticketHistoryEmptyState', true);
}

window.addEventListener('online', reconnectTicketHistoryOnResume);
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    console.log('[TICKET_REALTIME] APP BACKGROUND');
    cleanupTicketHistoryListener();
    return;
  }
...
window.addEventListener('pagehide', () => {
  console.log('[TICKET_REALTIME] PAGE HIDE');
  cleanupTicketHistoryListener();
});
window.addEventListener('beforeunload', () => {
  console.log('[TICKET_REALTIME] BEFORE UNLOAD');
  cleanupTicketHistoryListener();
});
...
if (typeof originalShowSection === 'function') {
  window.showSection = function(sectionId) {
    ...
    if (sectionId === 'ticket-history' && userEmail) {
      console.log('[TICKET_REALTIME] SECTION ACTIVE');
      cleanupTicketHistoryListener();
      loadTicketHistory('ticketHistory', 'ticketHistoryEmptyState', true);
    }
  };
