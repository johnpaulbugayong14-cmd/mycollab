...
let ticketHistoryRefreshInFlight = false;
...
function loadTicketHistory(containerId = "ticketHistory", emptyStateId = "ticketHistoryEmptyState", forceRefresh = false) {
  ...
  const refreshTicketHistory = async () => {
    if (ticketHistoryRefreshInFlight || !userEmail) return;
    ticketHistoryRefreshInFlight = true;
    try {
      ...
      ticketHistoryRefreshInFlight = false;
    }
  };

  void refreshTicketHistory();
  if (ticketHistoryRefreshTimer === null) {
    ticketHistoryRefreshTimer = setInterval(refreshTicketHistory, 1000);
  }
