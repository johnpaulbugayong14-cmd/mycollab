...
export async function requireAuth(allowedRoles = null) {
  ...
  // ONLY sign in anonymously if we genuinely have no user and no restricted role requirement
  if (!auth.currentUser && !storedUser) {
    try {
      await signInAnonymously(auth);
      console.log("requireAuth: Initialized anonymous fallback");
    } catch (error) {
      console.error("requireAuth: Failed to initialize fallback auth:", error);
    }
  }
