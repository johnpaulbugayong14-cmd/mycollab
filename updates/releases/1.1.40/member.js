...
window.submitTicket = async function () {
  ...
  try {
    console.log('[TICKET_CREATE] START', { userEmail, title, descriptionLength: description.length });
    ...
    console.log('[TICKET_CREATE] SUCCESS', { id: createdTicketRef?.id, submittedBy: userEmail, title });
...
function loadTicketHistory(containerId = "ticketHistory", emptyStateId = "ticketHistoryEmptyState", forceRefresh = false) {
  ...
  console.log('[TICKET_SYNC] LISTENER CREATE', {
    userEmail,
    listenerGeneration,
    collection: 'tickets',
    query: 'collection(db, "tickets")',
    platform: window.Capacitor ? window.Capacitor.getPlatform?.() || 'capacitor' : 'browser'
  });
  ticketHistoryUnsubscribe = onSnapshot(collection(db, "tickets"), (snapshot) => {
    console.log('[TICKET_SYNC] CALLBACK', {
      fromCache: snapshot.metadata?.fromCache,
      hasPendingWrites: snapshot.metadata?.hasPendingWrites,
      size: snapshot.size,
      ids: snapshot.docs.map(ticketDoc => ticketDoc.id),
      source: snapshot.metadata?.fromCache ? 'cache' : 'server'
    });
    snapshot.docChanges().forEach((change) => {
      console.log('[TICKET_SYNC] CHANGE', {
        type: change.type,
        id: change.doc.id,
    ...
    console.error('[TICKET_SYNC] ERROR', {
      code: error?.code,
