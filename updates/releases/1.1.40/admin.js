...
window.respondToTicket = async function(ticketId) {
  ...
  try {
    ...
    const ticketRefreshSignal = String(Date.now());
    localStorage.setItem('mycollab-ticket-refresh', ticketRefreshSignal);
    window.dispatchEvent(new Event('mycollab-ticket-refresh'));
...
window.changeTicketStatus = async function(ticketId, newStatus) {
  ...
  try {
    ...
    console.log('[ADMIN_TICKET_STATUS] UPDATE_START', { ticketId, newStatus, recipientEmail });
    await updateDoc(ticketRef, {
      status: newStatus,
      memberEmailNotificationSent: false,
    ...
    console.log('[ADMIN_TICKET_STATUS] UPDATE_SUCCESS', { ticketId, newStatus });
