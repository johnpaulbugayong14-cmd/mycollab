...
async function checkForUpdate(force = false) {
  if (window.Capacitor?.isNativePlatform?.()) {
    log('Native Android Capacitor build detected; skipping remote update checks to avoid stale app-shell replacement.');
    return;
  }
...
async function registerUpdater() {
  if (window.Capacitor?.isNativePlatform?.()) {
    log('Native Android Capacitor build detected; remote updater disabled.');
    return;
  }
