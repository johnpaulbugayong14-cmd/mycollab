...
function cleanupTicketHistoryListener() {
  ...
  try {
    ...
    console.warn('[TICKET_HISTORY_FETCH] Unable to clean up listener:', error);
  }
...
function loadTicketHistory(containerId = "ticketHistory", emptyStateId = "ticketHistoryEmptyState", forceRefresh = false) {
  ...
  if (ticketHistoryUnsubscribe) {
    if (forceRefresh) {
      console.log('[TICKET_HISTORY_FETCH] listener already active; keeping existing subscription while refreshing data');
    }
    return;
  }
  ...
  console.log('[TICKET_HISTORY_FETCH] creating fresh Firestore ticket listener');
  ticketHistoryUnsubscribe = onSnapshot(collection(db, "tickets"), (snapshot) => {
    if (listenerGeneration !== ticketHistoryListenerGeneration) return;
    const docs = [];
    snapshot.forEach(ticketDoc => docs.push(ticketDoc));
    latestTicketSnapshotDocs = docs;
    renderAllTicketHistory(docs);
    ...
    setTimeout(() => {
      if (userEmail && ticketHistoryTarget.containerId === containerId && !ticketHistoryUnsubscribe) {
...
function reconnectTicketHistoryOnResume() {
  ...
  console.log('[TICKET_HISTORY_FETCH] app resumed; rebuilding ticket history listener');
  cleanupTicketHistoryListener();
  loadTicketHistory('ticketHistory', 'ticketHistoryEmptyState', true);
}

window.addEventListener('online', reconnectTicketHistoryOnResume);
document.addEventListener('visibilitychange', reconnectTicketHistoryOnResume);
